...
names = ['Marko', 'Rasmus', 'Rainer', 'Maten', 'Martin'] # Sõitjad listis
...
def one_lap_time(mini, maxi, driver_name):
    ...
    this_total = 0 # Sektorid kokku liidetunud
...
def is_fastest_lap(driver_name, fastest_data):
    """ Kas on kiireima ringi aeg ja sõitja. Väljastus juures vaja. """
...
if __name__ == '__main__':
    ...
    for name in names: # Kõikide isikute tuleb teha allolev tegevus
        lap_times = 0 # Nulline isiku ringide arvu
        ...
        for lap in range(laps): # Hakkame sõitjale "ringe tegema"
            ...
            if random.randint(0, 9) == 2: # See on koperdatud ring
                ...
                this_lap = one_lap_time(minium, maximum, name)
            ...
            f.write(line + '\n') # Kirjuta rida faili revahetusega
